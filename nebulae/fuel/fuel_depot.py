...
class FuelDepot(object):
    def __init__(self):
        ...
        self.modifiable_keys = ['name', 'batch_size', 'height', 'width', 'resol_ratio',
                                'spatial_aug', 'p_sa', 'temporal_aug', 'p_ta']

    def loadFuel(self, config=None, name=None, batch_size=None, data_path=None, key_data=None,
                 if_shuffle=True, height=0, width=0, resol_ratio=1, is_seq=False,
        ...
        if config:
            ...
            config['p_sa'] = config.get('p_sa', list(p_sa))
            config['theta_sa'] = config.get('theta_sa', list(theta_sa))
            config['temporal_aug'] = config.get('temporal_aug', temporal_aug)
            config['p_ta'] = config.get('p_ta', list(p_ta))
            config['theta_ta'] = config.get('theta_ta', list(theta_ta))
        else:
            config = {}
            config['name'] = name
    ...
    def unloadFuel(self, name):
        name_err = Exception('%s is not an mounted dataset.' % name)
